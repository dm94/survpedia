...
1.  **Weight (Gravity):** Simple heavy objects (stones, blocks of concrete) that rely on friction and mass to hold the bottom. Effective for small boats or on hard bottoms.
2.  **Hooking (Shape):** The anchor is designed to dig into the seabed. As the ship pulls, the flukes (points) bury deeper. This is much more efficient than pure weight.

**The Catenary Effect:**
Crucial to anchoring is the "rode" (the rope or chain connecting anchor to ship). A heavy chain forms a curve (catenary). The weight of the chain keeps the pull on the anchor horizontal, which helps it dig in. If the pull is vertical, the anchor breaks out (trips).
